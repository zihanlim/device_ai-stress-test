...
# Simulates real AI engineering daily workflows with actual measurements
set -e
...
echo "=== Daily Usage Benchmark ===" | tee "$RESULTS_DIR/daily_usage_benchmark.txt"

DAILY_DATA="$RESULTS_DIR/daily_usage_data.txt"

# Write Python script to temp file
cat > /tmp/daily_usage_benchmark.py << 'PYEOF'
import time
import numpy as np
import subprocess
import sys
...
with open(DAILY_DATA, "a") as f:
    f.write(f"baseline,{0:.2f},{time.time():.2f},{0:.2f},{baseline_mem:.0f},{baseline_swap:.0f}\n")
...
for _ in range(10):
    _ = np.dot(browser[0], ide[:100, :100])
...
python3 /tmp/daily_usage_benchmark.py "$RESULTS_DIR" 2>&1 | tee -a "$RESULTS_DIR/daily_usage_benchmark.txt"

echo "" | tee -a "$RESULTS_DIR/daily_usage_benchmark.txt"
echo "Daily Usage Benchmark complete" | tee -a "$RESULTS_DIR/daily_usage_benchmark.txt"